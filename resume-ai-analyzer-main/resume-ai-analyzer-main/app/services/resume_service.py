...
'Researcher' or 'General Researcher' or 'Market Researcher' or 'Scientific Researcher': {
    ...
    for role, categories in keyword_categories.items():
        role_results = {}
...
def analyze_page_length(file_path: str) -> Dict:
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Error analyzing page length: {str(e)}")
...
def calculate_scores(text: str, sections: Dict, grammar_analysis: Dict, keyword_analysis: Dict) -> Dict:
    ...
    keyword_scores = [data['score'] for category in keyword_analysis.values() for data in category.values()]
    ...
    try:
        ...
        return {
            ...
            'improvement_suggestions': {
                ...
                'keywords': {cat: data['missing'] for cat, data in keyword_analysis.items()}
            }
        }
    except Exception as e:
